...
if (tamanho<=4):
	os.system('clear')
	print """
 	Programa de organizacao de imagens criado por Jorge Luiz Palmeiro Burger comecado no inicio 
de 2015.
...
utilizar, ou seja, recebe nome do projeto, sigla de estudo, numero da visita e ID do paciente.   
   ...
   Ou seja:
   	~
/DOWNLOADS_SUITE/ESCM001/RM/(volume\ 1 volume\ 2)
...
suite='/media/DATA/ESCMUL/DOWNLOADS_SUITE/'+folder
backup='/media/DATA/ESCMUL/ALINE/scripts/backup'
...
if Existe(scan) == False:
	Ir_para(scripts)
	for x in range(0,len(pastas)):
		os.system('fazer_scanlog_ALINE.py '+project+' '+pastas[x]+' '+folder+' '+visit)
		Ir_para(scripts)
